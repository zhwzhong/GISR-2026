...
class PBVS(nn.Module):
    def __init__(self, in_channels, embed_dim):
        ...
        self.head = nn.Sequential(
            ConvBNReLU2D(in_channels, out_channels=embed_dim, kernel_size=7, padding=3),
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.head = NIR(args.embed_dim) if args.dataset in ['NIR', 'NIR26'] else PBVS(args.in_channels, args.embed_dim)
        self.args = args
        enc_blks = [4, 6, 8, 8]
        middle_blk_num = 12
        dec_blks = [8, 8, 6, 4]
        ...
        if args.test_only and args.tlc_enhance:
            self.net = NAFNetLocal(img_channel=args.embed_dim, width=args.embed_dim, middle_blk_num=middle_blk_num,
                                   enc_blk_nums=enc_blks, base_size=base_size,
        ...
        self.tail = ConvBNReLU2D(in_channels=args.embed_dim, out_channels=args.in_channels, kernel_size=3, padding=1)


    def forward(self, samples):

        out = self.tail(self.net(self.head(samples)))
        return {'img_out': out + samples['lr_up']}
